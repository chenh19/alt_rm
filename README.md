...
- It is highly recommended to deploy this tool for manipulating files in **interactive** shells, especially on institutional servers as a ```regular``` user. For non-interactive shells (*i.e.*, scripts) and any other users without **alt_rm** deployed (including the ```root``` user), the original ```rm``` command will not be affected by the alias.
