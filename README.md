...
## Introduction
- This tool is intended to replace the Linux default ```rm``` command with **alias**, which deletes files directly and may cause unwanted troubles. 
...
- It is highly recommended to deploy this tool for manipulating files in **interactive** shells. For non-interactive shells (i.e., scripts), ```rm``` command``` will not be affected by the **alias**.
